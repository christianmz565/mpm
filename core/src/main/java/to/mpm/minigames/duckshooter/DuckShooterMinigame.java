...
/**
 * Minijuego tipo shooter donde los patos se disparan quacks entre sí.
 * Cada pato tiene 3 hits antes de ser eliminado.
 */
public class DuckShooterMinigame implements Minigame {
    private static final float SHOOT_COOLDOWN = 0.5f;
    private static final float GAME_DURATION = 120f; // 2 minutos
    ...
    public void initialize() {
        ...
        Gdx.app.log("DuckShooter", "Initialized local player " + localPlayerId + " as " + (nm.isHost() ? "HOST" : "CLIENT"));
    ...
    public void update(float delta) {
        ...
        // Verificar si solo queda un pato vivo
        int aliveDucks = 0;
        int lastAliveId = -1;
        for (IntMap.Entry<Duck> entry : ducks) {
        ...
        for (IntMap.Entry<Duck> entry : ducks) {
            ...
            if (quack.checkCollision(duck)) {
                ...
                if (damaged) {
                    ...
                    Gdx.app.log("DuckShooter", "HIT! Shooter=" + quack.shooterId + " Target=" + duck.playerId + " Remaining=" + duck.getHits());
    ...
    public void render(SpriteBatch batch, ShapeRenderer shapeRenderer) {
        ...
        String playerInfo = String.format("Player %d | Hits: %d | Kills: %d", 
            localPlayerId, 
            localDuck.getHits(), 
        ...
        int y = 380;
        font.draw(batch, "Scoreboard:", 450, y);
        y -= 25;
        ...
        for (Map.Entry<Integer, Integer> entry : sortedScores) {
            ...
            String status = duck != null && duck.isAlive() ? "ALIVE" : "DEAD";
            String scoreText = String.format("P%d: %d (%s)", entry.getKey(), entry.getValue(), status);
    ...
    private void shoot() {
        // Dirección hacia el mouse o hacia arriba por defecto
        Vector2 direction = new Vector2(0, 1);
        
        if (Gdx.input.getX() != 0 || Gdx.input.getY() != 0) {
            float mouseX = Gdx.input.getX();
            float mouseY = 480 - Gdx.input.getY(); // Invertir Y
            direction.set(mouseX - localDuck.position.x, mouseY - localDuck.position.y);
        }
    ...
    private void endGame() {
        ...
        // Encontrar ganador (más kills o último vivo)
        int maxScore = -1;
        for (Map.Entry<Integer, Integer> entry : scores.entrySet()) {
            Duck duck = ducks.get(entry.getKey());
            int score = entry.getValue();
            
            // Bonificación por estar vivo
        ...
        if (NetworkManager.getInstance().isHost()) {
            ...
            Gdx.app.log("DuckShooter", "Host sent GameEnd! Winner: " + winnerId);
        }
    }
    ...
    private void handleDuckState(DuckShooterPackets.DuckState state) {
        ...
        // Los clientes sí deben actualizar hits de DuckState para tener la vista correcta
        if (!NetworkManager.getInstance().isHost()) {
    ...
    private void handleShootQuack(DuckShooterPackets.ShootQuack shoot) {
        // Filtrar disparos del jugador local porque ya los creamos directamente en shoot()
        if (shoot.shooterId == localPlayerId) return;
